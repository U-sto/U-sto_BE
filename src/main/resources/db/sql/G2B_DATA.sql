...
CREATE TABLE TB_G2B001D (
  ...
  CRE_AT DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '레코드 생성 시점',
  UPD_BY VARCHAR(30) NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '조달청 정보 마지막 업데이트 주체',
  UPD_AT DATETIME NULL COMMENT '조달청 정보 마지막 업데이트 시각',

  KEY IDX_TB_G2B001D_M (G2B_M_CD), -- MySQL이 자동으로 만들어주겠지만, 혹시 모르니 FK 선언시에 명시적으로 인덱스를 만들었다
