use usto;

/*
 지금 flyway와 server가 Sync가 안 맞아서 오류가 발생합니다.
 그래서 싱크가 안 맞는 문제가 발생한다면
 SET FOREIGN_KEY_CHECKS = 0; -- 외래 키 검사 끄기
 DROP TABLE IF EXISTS flyway_schema_history; -- Flyway 실패기록 삭제
 DROP TABLE IF EXISTS TB_USER001M;
 DROP TABLE IF EXISTS TB_ORG002M;
 DROP TABLE IF EXISTS TB_ORG001M;
 DROP TABLE IF EXISTS TB_VERIF001M;
 DROP TABLE IF EXISTS TB_G2B_RAW;
 DROP TABLE IF EXISTS TB_G2B001D;
 DROP TABLE IF EXISTS TB_G2B001M;
 SET FOREIGN_KEY_CHECKS = 1;
 를 별도로 진행하시고 프로그램을 실행하셔야합니다. (그냥 커서 두고 실행하면 실행 됩니다)
 이후 잘 되면 더 이상 이 작업 필요 없습니다
 (flyway는 성공한 V_n은 더 이상 실행하지 않음)
 */

/*
[규칙]
1. 사용자는 반드시 하나의 조직에 속한다.
2. 조직은 사용자/운용부서의 상위 개념이다.
3. 조직의 조직코드는 <대학명>_<캠퍼스>
4. 운용부서 코드가 같아도 조직코드가 다르면 다른 운용부서 취급이다.
5. 운용부서는 복합키(기본키2개) 방식을 사용한다.
6. 운용부서의 운용부서코드는 <유형>_<팀>
*/

-- TB_ORG001M
CREATE TABLE `TB_ORG001M` (
  `ORG_CD` VARCHAR(50)      NOT NULL COMMENT '조직코드(의미코드)',
  `ORG_NM` VARCHAR(100)  NOT NULL COMMENT '조직명',
  `CRE_BY` VARCHAR(30)   NOT NULL COMMENT '생성자ID',
  `CRE_AT` DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일자',
  `UPD_BY` VARCHAR(30)   NULL COMMENT '수정자ID',
  `UPD_AT` DATETIME      NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일자',
  PRIMARY KEY (`ORG_CD`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO TB_ORG001M (
    ORG_CD,
    ORG_NM,
    CRE_BY
) VALUES
(
    'HYU_SEOUL',
    '한양대학교 서울캠퍼스',
    'SYSTEM'
),
(
    'HYU_ERICA',
    '한양대학교 ERICA캠퍼스',
    'SYSTEM'
);

-- 확인
SELECT *
FROM TB_ORG001M
ORDER BY ORG_CD;

-- TB_ORG002M
CREATE TABLE `TB_ORG002M` (
  `DEPT_CD` VARCHAR(50)      NOT NULL COMMENT '운용부서코드(의미코드, 조직 내 유일)',
  `ORG_CD`  VARCHAR(50)      NOT NULL COMMENT '조직코드(FK)',
  `DEPT_NM` VARCHAR(100)  NOT NULL COMMENT '운용부서명',
  `DEPT_TY` VARCHAR(20)   NOT NULL COMMENT '운용부서 유형',
  `CRE_BY`  VARCHAR(30)   NOT NULL COMMENT '생성자',
  `CRE_AT`  DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일자',
  `UPD_BY`  VARCHAR(30)   NULL COMMENT '수정자',
  `UPD_AT`  DATETIME      NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일자',
  PRIMARY KEY (`ORG_CD`, `DEPT_CD`), 
  CONSTRAINT `FK_TB_ORG002M_ORG`
    FOREIGN KEY (`ORG_CD`) REFERENCES `TB_ORG001M` (`ORG_CD`)
    ON UPDATE RESTRICT
    ON DELETE RESTRICT,
  KEY `IX_TB_ORG002M_DEPT_CD` (`DEPT_CD`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO TB_ORG002M (
    ORG_CD,
    DEPT_CD,
    DEPT_NM,
    DEPT_TY,
    CRE_BY
) VALUES
-- 총무관리처 계열
('HYU_ERICA', 'ADM_HR',     '총무인사팀',                 '총무관리처', 'SYSTEM'),
('HYU_ERICA', 'ADM_SAFE',   '캠퍼스안전팀',               '총무관리처', 'SYSTEM'),
('HYU_ERICA', 'ADM_ASSET',  '관재팀',                     '총무관리처', 'SYSTEM'),
('HYU_ERICA', 'ADM_FAC',    '시설팀',                     '총무관리처', 'SYSTEM'),

-- 단과대학 계열
('HYU_ERICA', 'COL_SW_RC',  '소프트웨어융합대학RC 행정팀', '단과대학',   'SYSTEM');

-- 확인
SELECT *
FROM TB_ORG002M
WHERE ORG_CD = 'HYU_ERICA'
ORDER BY DEPT_CD;

-- TB_USER001M
CREATE TABLE `TB_USER001M` (
  `USR_ID`  VARCHAR(30)   NOT NULL COMMENT '사용자ID(로그인ID, 회원가입 시 사용자가 설정)',
  `USR_NM`  VARCHAR(50)   NOT NULL COMMENT '사용자명',
  `PW_HASH` VARCHAR(255)  NOT NULL COMMENT '비밀번호(해시)',
  `EMAIL`   VARCHAR(50)   NOT NULL COMMENT '이메일(UNIQUE)',
  `PHONE`  CHAR(11)      NOT NULL COMMENT '전화번호',
  `ROLE_ID` VARCHAR(20)   NOT NULL DEFAULT 'GUEST' COMMENT '역할ID(ADMIN,MANAGER,GUEST)',
  `APPR_STS` VARCHAR(20) NOT NULL DEFAULT 'WAIT' COMMENT '승인여부(APPROVED,WAIT,REJECTED,SUSPENDED)',
  `APPR_USR_ID` VARCHAR(30) NULL COMMENT '승인자ID',
  `APPR_AT` DATETIME NULL COMMENT '승인일자',
  `ORG_CD`  VARCHAR(50)      NOT NULL COMMENT '조직코드(FK)',
  `CRE_BY`  VARCHAR(30)   NOT NULL COMMENT '생성자ID',
  `CRE_AT`  DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일자',
  `UPD_BY`  VARCHAR(30)   NULL COMMENT '수정자ID',
  `UPD_AT`  DATETIME      NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일자',
  PRIMARY KEY (`USR_ID`),
  UNIQUE KEY `UK_TB_USER001M_EMAIL` (`EMAIL`),
  CONSTRAINT `FK_TB_USER001M_ORG`
    FOREIGN KEY (`ORG_CD`) REFERENCES `TB_ORG001M` (`ORG_CD`)
    ON UPDATE RESTRICT
    ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

/*
 테스트용 유저 데이터
 "비밀번호는 전부 1111 "
 */

/* 1) 관리자(승인 완료) : 로그인 성공 + ADMIN 권한 테스트 */
INSERT INTO TB_USER001M
    (
     USR_ID,
     USR_NM,
     PW_HASH,
     EMAIL,
     PHONE,
     ROLE_ID,
     APPR_STS,
     APPR_USR_ID,
     APPR_AT,
     ORG_CD,
     CRE_BY
    )VALUES
    ('admin01', '관리자',
     '$2b$10$N9qo8uLOickgx2ZMRZo5i.ej3xGJdZQzQ0jrISFRCGDpa2BkLomf6',
     'admin01@usto.com', '01011111111',
     'ADMIN', 'APPROVED',
     'SYSTEM', NOW(),
     'HYU_ERICA', 'SYSTEM'),

/* 2) 일반 사용자(승인 완료) : 정상 로그인 성공 케이스 */
    ('manager01', '물품담당관',
     '$2b$10$N9qo8uLOickgx2ZMRZo5i.ej3xGJdZQzQ0jrISFRCGDpa2BkLomf6',
     'user_ok@usto.com', '01022222222',
     'MANAGER', 'APPROVED',
     'admin01', NOW(),
     'HYU_ERICA', 'SYSTEM'),

/* 3) 승인 대기(WAIT) : 로그인 차단(Disabled/승인안됨) 케이스 */

    ('guest01', '승인대기유저',
     '$2b$10$N9qo8uLOickgx2ZMRZo5i.ej3xGJdZQzQ0jrISFRCGDpa2BkLomf6',
     'user_wait@usto.com', '01033333333',
     'GUEST', 'WAIT',
     NULL, NULL,
     'HYU_ERICA', 'SYSTEM');


-- 반려는 넣을까 고민했지만, 굳이 싶어서(담당관 많아야 2명임) 주석처리
/* 4) 반려(REJECTED) : 로그인 차단 케이스(Disabled/승인반려됨)
INSERT INTO TB_USER001M
(USR_ID, USR_NM, PW_HASH, EMAIL, PHONE, ROLE_ID, APPR_STS, APPR_USR_ID, APPR_AT, ORG_CD, CRE_BY)
VALUES
    ('rejected01', '반려유저', '$2b$10$N9qo8uLOickgx2ZMRZo5i.ej3xGJdZQzQ0jrISFRCGDpa2BkLomf6',
     'user_rej@usto.com', '01000000004', 'GUEST', 'REJECTED',
     NULL, NULL, 'HANYANG_ERICA', 'SYSTEM');
*/

-- 검증
SELECT *
FROM TB_USER001M;

-- 최종 확인
SHOW TABLES;
