package com.usto.api.item.operation.domain.service;

import com.usto.api.common.exception.BusinessException;
import com.usto.api.item.common.model.ApprStatus;
import com.usto.api.item.operation.domain.model.OperationMaster;
import org.springframework.stereotype.Component;

import java.time.LocalDate;
import java.time.ZoneId;

/**
 * Operation 도메인의 비즈니스 정책
 */
@Component
public class OperationPolicy {

    private static final ZoneId KOREA_ZONE = ZoneId.of("Asia/Seoul");

    /**
     * 조직 소유권 검증
     */
    public void validateOwnership(OperationMaster master, String requestOrgCd) {
        if (!master.getOrgCd().equals(requestOrgCd)) {
            throw new BusinessException("해당 조직의 데이터가 아닙니다.");
        }
    }

    /**
     * 수정/삭제 가능 여부 검증
     */
    public void validateModifiable(OperationMaster master) {
        if (master.getApprSts() != ApprStatus.WAIT) {
            throw new BusinessException("승인 요청 중이거나 확정된 데이터는 수정/삭제할 수 없습니다.");
        }
    }

    /**
     * 승인 요청 가능 여부 검증
     */
    public void validateRequestable(OperationMaster master) {
        if (master.getApprSts() != ApprStatus.WAIT) {
            throw new BusinessException("승인요청이 가능한 상태가 아닙니다.");
        }
    }

    /**
     * 승인 취소 가능 여부 검증
     */
    public void validateCancellable(OperationMaster master) {
        if (master.getApprSts() != ApprStatus.REQUEST) {
            throw new BusinessException("승인요청 중인 상태만 취소할 수 있습니다.");
        }
    }

    /**
     * 운용일자 검증
     */
    public void validateAplyAt(LocalDate aplyAt) {
        if (aplyAt == null || aplyAt.isAfter(LocalDate.now(KOREA_ZONE))) {
            throw new BusinessException("운용일자는 미래 날짜를 선택할 수 없습니다.");
        }
    }
}